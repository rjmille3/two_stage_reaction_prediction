...
from rpCHEM.Common.MolExt import clearAtomMaps, hasAtomMaps, aromatizeMol

from rpCHEM.Common.Util import log
from rpCHEM.Common.Util import createStdAtomMapSmiString, splitCompositeSmilesToList, joinSmilesListToCompositeSmiles
    ...
    for atom in nMol.GetAtoms():
        if atom.GetMapIdx() in breadCrumb:
            for nMapIdx in breadCrumb[atom.GetMapIdx()]:
                ...
                #print("current implicit h count: ", atom.GetImplicitHCount())
...
def createPossibleAtomMapSmiles(smi):
    ...
    #print("hatommaps param ", hAtomMaps)
    breadCrumb = {}
    if hAtomMaps:
    ...
    #ss.SetMaxMatches(0)  # we get a lot of "max matches reached" warnings, set 0 to uncap
    ss.SetMaxMatches(2048) 
    clearAtomMaps(mol)    
